...
запуском, сохранение/загрузка и инспектор субъективного состояния агента. Карточка
персонажа показывает его модель, русское публичное намерение, руководство, поле зрения,
память и отношения; модель можно заменить без потери личности. Режиссёр мира позволяет
явно и с записью в хронику добавить существо или запустить одно из событий. Браузер
получает WebSocket-снимки, но все команды проверяются авторитетным сервером. В свободном
мире первые жители появляются в удалённых друг от друга областях карты.
...
Интерфейс объединяет модели, уже подключённые в локальном Ollama, с актуальным официальным
облачным каталогом `https://ollama.com/api/tags`. Каталог обновляется без жёсткого списка;
если официальный сервис временно недоступен, остаются локально подключённые модели. При
первом назначении облачная модель активируется через локальный авторизованный Ollama runtime:
